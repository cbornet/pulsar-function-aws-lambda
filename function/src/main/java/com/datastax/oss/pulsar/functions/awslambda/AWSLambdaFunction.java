...
public class AWSLambdaFunction extends AbstractAwsConnector
    ...
    public Record<GenericObject> process(GenericObject input, Context context) throws Exception {
        try {
            ...
            if (result != null && result.getStatusCode() < 300 && result.getStatusCode() >= 200) {
                ...
                LambdaResponse response = MAPPER.readValue(arr, LambdaResponse.class);
                JsonRecord jsonRecord = response.getBody();

                Schema outputSchema;
                Object outputValue;
                if (jsonRecord.getValue().getSchemaType().equals(SchemaType.KEY_VALUE)) {
                    KeyValueTypedValue keyValue = (KeyValueTypedValue) jsonRecord.getValue();
                    Schema<?> keyOutputSchema = getSchema(keyValue.getValue());
                    Schema<?> valueOutputSchema = getSchema(keyValue.getKey());
                    if (jsonRecord.getKeyValueEncodingType() != null) {
                        outputSchema = Schema.KeyValue(keyOutputSchema, valueOutputSchema, jsonRecord.getKeyValueEncodingType());
                    } else {
                        outputSchema = Schema.KeyValue(keyOutputSchema, valueOutputSchema);
                    }
                    outputValue = new KeyValue<>(keyValue.getKey().getValue(), keyValue.getValue().getValue());
                } else {
                ...
                logger
                    .error(
                        "failed to send message to AWS Lambda function {}.",
                        config.getLambdaFunctionName());
    ...
    private TypedValue<?> getTypedValue(Schema<?> schema, Object value) throws IOException {
        ...
        switch (schema.getSchemaInfo().getType()) {
            ...
            case KEY_VALUE:
                ...
                TypedValue<?> keyTypedValue = getTypedValue(keyValueSchema.getKeySchema(), keyValue.getKey());
                TypedValue<?> valueTypedValue = getTypedValue(keyValueSchema.getValueSchema(), keyValue.getValue());
    ...
    private static byte[] serializeAvro(org.apache.avro.generic.GenericRecord record) throws IOException {
        GenericDatumWriter<org.apache.avro.generic.GenericRecord> writer = new GenericDatumWriter<>(record.getSchema());
    ...
    private Schema<?> getSchema(TypedValue<?> typedValue) {
        switch (typedValue.getSchemaType()) {
            case AVRO:
                org.apache.avro.Schema.Parser parser = new org.apache.avro.Schema.Parser();
                org.apache.avro.Schema schema = parser.parse(new String(typedValue.getSchema(), StandardCharsets.UTF_8));
                return Schema.NATIVE_AVRO(schema);
            case JSON:
                return Schema.JSON(JsonNode.class);
            case INT8:
                return Schema.INT8;
            case INT16:
                return Schema.INT16;
            ...
            default:
                throw new IllegalStateException(
                    String.format("Function output schema type %s not supported at the moment", typedValue.getSchemaType()));
        }
    }
